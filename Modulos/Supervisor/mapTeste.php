...
<html>
    <head>
        ...
        <style>
            ...
            #map { position:absolute; top:0em; bottom:0; width:100%; }
        </style>
        <script>
            function movevtr() {
                ...
                var iconVtr = L.icon({
                    iconUrl: Base_URL+'/Templates/imgs/pin_brown.svg',
                    iconSize: [30, 47]});
                ...
                fetch(url, {cache: "no-cache"})
                        ...
                        .then(data => {
                            ...
                            for (i = 0; i < data.length; i++) {
                                ...
                                marker = new L.marker([data[i].latitude, data[i].longitude], {icon: iconVtr}).addTo(map).bindPopup('Viatura: ' + data[i].prefixo
...
//tm_raster = setTimeout(addvtr, 15000);
                tm_raster = setTimeout(movevtr, 15000);
            }
        </script>
    </head>
    <body>
        <div id='map'></div>
        <script>
            L.mapbox.accessToken = 'pk.eyJ1Ijoic2xhY2tyaWNoYXJkIiwiYSI6ImNra2lrbTM3ajA4bnAyb216dW5xMHJwN2cifQ.i87Cu90h_saOi786d0ktcA';
            //L.mapbox.accessToken = 'pk.eyJ1Ijoic2xhY2tyaWNoYXJkIiwiYSI6ImNra2lyaDRrcDBtZXEyd3BrNTBpczV1OGcifQ.WJcQ-_B9L6lwgxL6J2l_Bg';

            var map = L.mapbox.map('map').setView([-14.882630, -40.863624], 10);
            L.control.layers({
                'Mapbox Ruas': L.mapbox.styleLayer('mapbox://styles/mapbox/streets-v11').addTo(map),
                'Mapbox Leve': L.mapbox.styleLayer('mapbox://styles/mapbox/light-v10'),
                'Mapbox Outdoors': L.mapbox.styleLayer('mapbox://styles/mapbox/outdoors-v11'),
                'Mapbox Satelite': L.mapbox.styleLayer('mapbox://styles/mapbox/satellite-v9'),
                'Mapbox Ruas-Satelite': L.mapbox.styleLayer('mapbox://styles/mapbox/satellite-streets-v11')
            }).addTo(map);
            // poligono Bahia
            ...
            L.geoJson(bahia, {style: L.mapbox.simplestyle.style}).addTo(map);
            ...
            L.geoJson(vitcon, {style: L.mapbox.simplestyle.style}).addTo(map);
