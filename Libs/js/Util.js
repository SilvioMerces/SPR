...
tm_raster = null;//variavel para ratreamento de viatura
...
function addvtr() {
    ...
    var iconVtr = L.icon({
        iconUrl: 'Templates/imgs/icon_vtr.svg',
        iconSize: [30, 47]});

    mymap.eachLayer(function (layer) {
        if (layer instanceof L.Marker) {if (mymap.getBounds().contains(layer.getLatLng())) {layer.remove();}}
    });

    url = Base_URL + '/Modulos/TrackingVtr/marker_vrt_patrulha.php';

    var duashorasantes = new Date();
    duashorasantes.setHours(duashorasantes.getHours() - 2);
    
    if (url.indexOf('?') > 0) {url += '&t=' + duashorasantes;}
    ...
    fetch(url, {cache: "no-cache"})
            ...
            .then(data => {
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    marker = new L.marker([data[i].latitude, data[i].longitude],{icon:iconVtr}).addTo(mymap).bindPopup('Viatura: '+data[i].prefixo
                            +'<br>Latitude:'+data[i].latitude+',Longitude:'+data[i].longitude
                            +',<br>Data/Hora:'+data[i].dth
                            +'<br>Telefone :'+data[i].nrfone);
                }
...
//tm_raster = setTimeout(addvtr, 15000);
  tm_raster = setTimeout(addvtr, 5000);
}
